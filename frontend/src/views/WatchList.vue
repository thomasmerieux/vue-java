    ...
    <div v-for="(item, index) in movies" :key="item.id" class="card mb-4" :class="{ 'mt-3': index === 0 }"
      ...
      <MovieListItem v-bind:item="item">
        <template v-slot:actions>
          <div class="pr-1" v-if="item.isFavorite">
            <IconAndText class="pr-4" lib="fas" icon="heart" label="Favorite"
                         v-on:clicked="removeFavorite(item.id, index)"></IconAndText>
          </div>
          <div class="pr-1" v-if="!item.isFavorite">
            <IconAndText class="pr-4" lib="far" icon="heart" label="Favorite"
                         v-on:clicked="addToFavorite(item.id, index)"></IconAndText>
          </div>
          <IconAndText class="pr-4" lib="fas" icon="list" label="Add to list"
                       v-on:clicked="showModal(item)"></IconAndText>
          <IconAndText class="pr-4" lib="far" icon="times-circle" label="Remove from watch list"
                       v-on:clicked="removeFromWatchlist(item.id)"></IconAndText>
        </template>
      </MovieListItem>
    </div>
  </template>
  ...
  <Modal v-show="isModalVisible"
    ...
    <template v-slot:body>
      ...
      <div class="d-flex align-items-center justify-content-center mt-5">
        <IconAndText class="pr-4" lib="fas" icon="plus-circle" label="Create a new list"
                     v-on:clicked="createList()"></IconAndText>
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    async getFavorites() {
      const res = await fetch(`http://localhost:8080/watch-list`);
    ...
    async removeFromWatchlist(id) {
      ...
      await fetch(`http://localhost:8080/watch-list/${id}`, requestOptions).then(() => {
        this.getFavorites();
      });
    },
    async removeFavorite(id, index) {
      ...
      await fetch(`http://localhost:8080/favorite/${id}`, requestOptions).then(async () => {
        this.movies[index] = await this.getDetail(id);
      });
    },
    async addToFavorite(id, index) {
      const requestOptions = {
        method: "POST",
        headers: {"Content-Type": "application/json"}
      };
